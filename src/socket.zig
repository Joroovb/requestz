...
fn SocketWrapper(comptime Engine: type) type {
    return struct {
        ...
        pub fn connect(allocator: *const Allocator, uri: Uri) !Self {
        ...
        fn connectToHost(allocator: *const Allocator, host: []const u8, port: u16) !Engine.Socket {
            return try Engine.connectToHost(allocator, host, port, .tcp);
        }

        fn connectToAddress(_: *const Allocator, address: Address) !Engine.Socket {
...
const ZigNetwork = struct {
    ...
    fn connectToHost(allocator: *const Allocator, host: []const u8, port: u16, protocol: network.Protocol) !Socket {
...
const NetworkMock = struct {
    ...
    pub fn connectToHost(allocator: *const Allocator, host: []const u8, port: u16, protocol: network.Protocol) !Socket {
        _ = allocator;
        _ = host;
        _ = port;
        _ = protocol;
